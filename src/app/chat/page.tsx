...
type KanalBildirim = { [kanalId: string]: number }
...
export default function Chat() {
  ...
  const [alinti, setAlinti] = useState<{ icerik: string; gonderen: string } | null>(null)
  const [swipeMesajId, setSwipeMesajId] = useState<string | null>(null)
  ...
  useEffect(() => {
    async function yukle() {
      ...
      const { data: profil } = await supabase.from('profiles').select('rol, email').eq('id', user.id).single()
      const { data: karakter } = await supabase.from('karakterler').select('karakter_adi').eq('kullanici_id', user.id).single()
      ...
      const { data: kanallarData } = await supabase.from('chat_kanallari').select('*').eq('aktif', true).order('created_at')
      setKanallar(kanallarData ?? [])

      const { count } = await supabase.from('private_mesajlar')
        .select('*', { count: 'exact', head: true }).eq('alici_id', user.id).eq('okundu', false)
      setOkunmamis(count ?? 0)

      const { data: profillerData } = await supabase.from('profiles').select('id, email, rol').neq('id', user.id)
      const { data: karakterlerData } = await supabase.from('karakterler').select('kullanici_id, karakter_adi')
      setKullanicilar((profillerData ?? []).map(p => ({
        ...p,
  ...
  useEffect(() => {
    ...
    const globalKanal = supabase.channel('global_mesajlar')
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'chat_mesajlari' }, (payload) => {
        const yeni = payload.new as Mesaj
        if (yeni.kullanici_id !== kullaniciRef.current?.id && yeni.kanal_id !== aktifKanalRef.current?.id) {
          setKanalBildirim(prev => ({ ...prev, [yeni.kanal_id]: (prev[yeni.kanal_id] ?? 0) + 1 }))
        }
        if (yeni.kanal_id === aktifKanalRef.current?.id) {
          setMesajlar(prev => prev.find(m => m.id === yeni.id) ? prev : [...prev, yeni])
        }
      })
      .subscribe()

    const privateKanal = supabase.channel(`private_bildirim_${kullanici.id}`)
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'private_mesajlar', filter: `alici_id=eq.${kullanici.id}` }, (payload) => {
        const yeni = payload.new as PrivateMesaj
        setOkunmamis(prev => prev + 1)
        setPrivateMesajlar(prev => {
          if (!prev.length) return prev
          return prev.find(m => m.id === yeni.id) ? prev : [...prev, yeni]
        })
      })
      // Görüldü güncellemelerini dinle
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'private_mesajlar', filter: `gonderen_id=eq.${kullanici.id}` }, (payload) => {
        const guncellenen = payload.new as PrivateMesaj
        setPrivateMesajlar(prev => prev.map(m => m.id === guncellenen.id ? { ...m, goruldu: guncellenen.goruldu, goruldu_at: guncellenen.goruldu_at } : m))
      })
      .subscribe()
  ...
  useEffect(() => {
    ...
    supabase.from('chat_mesajlari').select('*').eq('kanal_id', aktifKanal.id)
  ...
  useEffect(() => {
    ...
    const fetchPrivate = async () => {
      const { data } = await supabase.from('private_mesajlar').select('*')
      ...
      // Okundu + görüldü işaretle
      await supabase.from('private_mesajlar')
        .update({ okundu: true, goruldu: true, goruldu_at: new Date().toISOString() })
        .eq('gonderen_id', privateHedef.id).eq('alici_id', kullanici.id).eq('okundu', false)
  ...
  // Alıntı seçilince input'a focus
  useEffect(() => {
    if (alinti) inputRef.current?.focus()
  }, [alinti])
  ...
  async function mesajGonder() {
    ...
    if (data && !mesajlar.find(m => m.id === data.id)) setMesajlar(prev => [...prev, data as Mesaj])
    if (error) setYeniMesaj(icerik)
    setGonderiyor(false)
  }
  ...
  async function privateMesajGonder() {
    ...
    if (data && !privateMesajlar.find(m => m.id === data.id)) setPrivateMesajlar(prev => [...prev, data as PrivateMesaj])
    if (error) setOzelMesaj(icerik)
    setGonderiyor(false)
  }

  // Swipe handlers
  function swipeBaşla(e: React.TouchEvent, id: string) {
    swipeStartX.current = e.touches[0].clientX
    swipeStartY.current = e.touches[0].clientY
    setSwipeMesajId(id)
  }

  function swipeHareket(e: React.TouchEvent) {
    const dx = e.touches[0].clientX - swipeStartX.current
    const dy = Math.abs(e.touches[0].clientY - swipeStartY.current)
    if (dy > 20) { setSwipeX(0); return } // Dikey kaydırma — iptal
    if (dx > 0 && dx < 80) setSwipeX(dx)
  }

  function swipeBitis(m: Mesaj | PrivateMesaj) {
    if (swipeX > 50) {
      ...
      setAlinti({ icerik: m.icerik.slice(0, 80), gonderen: gonderenAdi })
    }
    setSwipeX(0)
    setSwipeMesajId(null)
  }

  function kanalSec(kanal: Kanal) {
    setAktifKanal(kanal); setMod('kanal'); setPrivateHedef(null)
    setKanalBildirim(prev => ({ ...prev, [kanal.id]: 0 }))
    setSidebarAcik(false); setAlinti(null)
  }
  ...
  // Mesaj içeriğini alıntı ve normal parça olarak render et
  function mesajIcerikRender(icerik: string, benimMi: boolean) {
    const satirlar = icerik.split('\n')
    const alıntiSatirlari: string[] = []
    const normalSatirlar: string[] = []
    let alıntiModu = true

    for (const satir of satirlar) {
      if (alıntiModu && satir.startsWith('> ')) {
        alıntiSatirlari.push(satir.slice(2))
      } else {
        alıntiModu = false
        if (satir !== '') normalSatirlar.push(satir)
      }
    }

    return (
      <div className="flex flex-col gap-1">
        {alıntiSatirlari.length > 0 && (
          <div className="px-3 py-1.5 text-xs leading-relaxed opacity-60 border-l-2"
            style={{ borderColor: benimMi ? 'rgba(168,85,247,0.6)' : 'rgba(255,255,255,0.3)', background: benimMi ? 'rgba(168,85,247,0.08)' : 'rgba(255,255,255,0.04)', borderRadius: '4px' }}>
            {alıntiSatirlari.join('\n')}
          </div>
        )}
        {normalSatirlar.length > 0 && (
          <span>{normalSatirlar.join('\n')}</span>
        )}
      </div>
    )
  }
  ...
  return (
    <main className="h-screen bg-black flex flex-col overflow-hidden relative">
      <div className="fixed inset-0 opacity-10" style={{ backgroundImage: "url('/theia-bg.jpg')", backgroundSize: 'cover' }} />
      ...
      <div className="relative z-10 flex h-full overflow-hidden">
        ...
        {sidebarAcik && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-40 md:hidden" onClick={() => setSidebarAcik(false)} />
        )}

        {/* SOL SIDEBAR */}
        <div className={`fixed inset-y-0 left-0 z-50 w-72 bg-black border-r border-white/5 transition-transform duration-300 flex flex-col
          ${sidebarAcik ? 'translate-x-0' : '-translate-x-full'} md:relative md:translate-x-0 md:w-64 md:shrink-0`}>
          ...
          <div className="flex flex-col gap-1 p-3 border-b border-white/5">
            ...
            {kanallar.map(kanal => {
              ...
              return (
                <button key={kanal.id} onClick={() => erisim && kanalSec(kanal)}
                  className="flex items-center gap-3 px-3 py-2.5 text-left transition-all rounded"
                  style={{ background: aktifKanal?.id === kanal.id ? 'rgba(168,85,247,0.15)' : 'transparent', borderLeft: aktifKanal?.id === kanal.id ? '2px solid rgba(168,85,247,0.6)' : '2px solid transparent', opacity: erisim ? 1 : 0.3 }}>
          ...
          <div className="flex flex-col gap-1 p-3 flex-1 overflow-y-auto">
            ...
            {kullanicilar.map(k => (
              <button key={k.id}
                onClick={() => { setPrivateHedef(k); setMod('private'); setAktifKanal(null); setSidebarAcik(false); setAlinti(null) }}
                className="flex items-center gap-3 px-3 py-2 text-left transition-all rounded"
                style={{ background: privateHedef?.id === k.id ? 'rgba(34,211,238,0.1)' : 'transparent', borderLeft: privateHedef?.id === k.id ? '2px solid rgba(34,211,238,0.5)' : '2px solid transparent' }}>
                ...
                <div className="flex flex-col gap-0.5 min-w-0">
                  ...
                  <span style={{ color: `${rolRenk[k.rol]}80`, fontSize: '10px' }}>{rolEtiket[k.rol]}</span>
                </div>
              </button>
            ))}
          </div>
          ...
          <div className="px-5 py-3 border-t border-white/5">
            <Link href="/portal" className="text-white/15 text-xs tracking-widest uppercase hover:text-white/40 transition-all">← Portal</Link>
          </div>
        </div>
        ...
        <div className="flex-1 flex flex-col overflow-hidden bg-black/20">
          ...
          <div className="px-4 md:px-6 py-4 border-b border-white/5 shrink-0 flex items-center gap-3 bg-black/40">
            <button onClick={() => setSidebarAcik(true)} className="md:hidden p-2 -ml-2 text-fuchsia-400/60">
            ...
            {aktifKanal ? (
              <>
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-white/20 text-xs truncate">{aktifKanal.aciklama}</p>
                </div>
              </>
            ) : privateHedef ? (
              <>
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-xs truncate" style={{ color: `${rolRenk[privateHedef.rol]}80` }}>{rolEtiket[privateHedef.rol]}</p>
                </div>
              </>
            ) : (
          ...
          <div className="flex-1 overflow-y-auto px-4 md:px-6 py-4 flex flex-col gap-2"
            ...
            {(aktifKanal || privateHedef) ? aktifMesajlar.map((m, i) => {
              ...
              const suankiG = mod === 'kanal' ? (m as Mesaj).kullanici_id : (m as PrivateMesaj).gonderen_id
              const oncekiG = onceki ? (mod === 'kanal' ? (onceki as Mesaj).kullanici_id : (onceki as PrivateMesaj).gonderen_id) : null
              const ayniGonderen = oncekiG === suankiG
              const buTarih = tarihStr(m.created_at)
              const yeniGun = onceki ? tarihStr(onceki.created_at) !== buTarih : true

              // Son mesaj ve benim mesajım ise görüldü göster
              const sonMesajMi = i === aktifMesajlar.length - 1
              const gorulduGoster = mod === 'private' && benimMi && sonMesajMi && (m as PrivateMesaj).goruldu

              const swipeAktif = swipeMesajId === m.id
              ...
              return (
                <div key={m.id}>
                  ...
                  <div
                    className={`flex gap-2 md:gap-3 ${benimMi ? 'flex-row-reverse' : 'flex-row'}`}
                    onTouchStart={e => swipeBaşla(e, m.id)}
                    onTouchMove={swipeHareket}
                    onTouchEnd={() => swipeBitis(m)}
                    style={{ transform: swipeAktif && !benimMi ? `translateX(${swipeX}px)` : 'none', transition: swipeAktif ? 'none' : 'transform 0.2s' }}
                  >
                    {/* Swipe ikonu */}
                    {swipeAktif && swipeX > 20 && !benimMi && (
                      <div className="flex items-center text-fuchsia-400/60 text-xs mr-1" style={{ opacity: swipeX / 60 }}>↩</div>
                    )}
                    ...
                    <div className={`flex flex-col gap-0.5 max-w-[85%] md:max-w-md ${benimMi ? 'items-end' : 'items-start'}`}>
                      {!ayniGonderen && (
                        <div className={`flex items-baseline gap-2 mb-0.5 ${benimMi ? 'flex-row-reverse' : ''}`}>
                          <span className="text-xs font-medium" style={{ color: rolRenk[gonderenRol] ?? '#6b7280' }}>{gonderenAdi}</span>
                          <span className="text-white/15 text-xs">{zaman(m.created_at)}</span>
                        </div>
                      )}

                      {/* Alıntı ile cevap için sağ tık / uzun basma alternatifi olarak çift tıklama */}
                      <div
                        className="px-3 md:px-4 py-2 text-sm leading-relaxed break-words cursor-pointer select-none"
                        onDoubleClick={() => {
                          setAlinti({ icerik: m.icerik.slice(0, 80), gonderen: gonderenAdi })
                        }}
                        style={{
                        ...
                        {mesajIcerikRender(m.icerik, benimMi)}
                      </div>

                      {ayniGonderen && <span className="text-white/10 text-xs mt-0.5">{zaman(m.created_at)}</span>}
                      ...
                      {gorulduGoster && (
                        <div className="flex items-center gap-1 mt-0.5">
                          ...
                          <span className="text-white/15 text-xs">
                            Görüldü {(m as PrivateMesaj).goruldu_at ? zaman((m as PrivateMesaj).goruldu_at!) : ''}
                          </span>
                        </div>
                      )}
                    </div>
          ...
          {(aktifKanal || privateHedef) && (
            <div className="px-4 md:px-6 py-3 border-t border-white/5 bg-black/60 shrink-0">
              {/* Alıntı önizleme */}
              {alinti && (
                <div className="flex items-start gap-2 mb-2 px-3 py-2 rounded"
                  style={{ background: 'rgba(168,85,247,0.08)', borderLeft: '2px solid rgba(168,85,247,0.5)' }}>
                  <div className="flex-1 min-w-0">
                    <p className="text-fuchsia-400/60 text-xs mb-0.5">{alinti.gonderen}'e yanıt</p>
                    <p className="text-white/40 text-xs truncate">{alinti.icerik}</p>
                  </div>
                  <button onClick={() => setAlinti(null)} className="text-white/20 hover:text-white/60 text-lg shrink-0">×</button>
                </div>
              )}

              <div className="flex items-end gap-2 bg-white/[0.03] border border-white/10 rounded-lg p-2 md:p-3">
                <textarea
                  ref={inputRef}
                  ...
                  placeholder={alinti ? `${alinti.gonderen}'e yanıt yaz...` : 'Mesajını yaz...'}
                  rows={1}
