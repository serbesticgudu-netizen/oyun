...
export default function OyunArkadaslari() {
  ...
  if (yukleniyor) {
    return (
      <main className="min-h-screen bg-black flex items-center justify-center">
        <p className="text-white/20 text-xs tracking-widest uppercase animate-pulse">Oyun arkadaşları aranıyor...</p>
      </main>
    )
  }
  ...
  return (
    <main className="min-h-screen bg-black relative">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 md:px-8 py-10 md:py-14 flex flex-col gap-8 md:gap-10">
        
        {/* DÜZELTİLMİŞ BAŞLIK ALANI (Yataydan Dikeye Esneyen Düzen) */}
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-white/5 pb-6 shrink-0">
          <div className="flex flex-col gap-2">
            <p className="text-fuchsia-400/30 text-[10px] md:text-xs tracking-[0.5em] uppercase">Theia Kabilesi</p>
            <h1 className="text-white text-3xl md:text-4xl font-thin tracking-[0.3em] uppercase" style={{ textShadow: '0 0 30px rgba(168,85,247,0.3)' }}>
              Oyun Arkadaşları
            </h1>
            <p className="text-white/20 text-xs tracking-wider">{filtrelenmisKarakterler.length} varlık bulundu.</p>
          </div>
          <Link href="/portal" className="border border-white/10 text-white/30 hover:border-white/30 hover:text-white/60 px-4 py-2 text-xs tracking-widest uppercase transition-all bg-black/40 w-fit">
            ← Portal
          </Link>
        </div>
        ...
        <div className="flex flex-col md:flex-row gap-4">
          <input
            ...
            className="flex-1 bg-black/30 border border-white/10 px-4 py-2 text-white text-sm placeholder-white/30 focus:outline-none focus:border-fuchsia-500/50 rounded"
          />
...
<button onClick={() => setSeciliKarakter(null)} className="text-white/40 hover:text-white transition-colors text-2xl">×</button>
            ...
            <div className="p-6 flex-1 overflow-y-auto flex flex-col gap-5">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                {/* SOL KOLON — Fotoğraf ve "Profili Görüntüle" Butonu */}
                <div className="md:col-span-1 flex flex-col gap-3">
                  {seciliKarakter.gorsel_url && (
                    <div className="relative w-full aspect-[4/5] bg-black/20 border border-white/10 rounded-md overflow-hidden shrink-0">
                      <img src={seciliKarakter.gorsel_url} alt={seciliKarakter.karakter_adi} className="w-full h-full object-contain" />
                    </div>
                  )}
                  {/* YENİ: DİNAMİK PROFİL GÖRÜNTÜLEME BUTONU */}
                  <Link 
                    href={`/portal/${seciliKarakter.kullanici_id}`}
                    className="border border-fuchsia-500/40 text-fuchsia-300 px-4 py-2 text-[10px] tracking-widest uppercase hover:bg-fuchsia-500/10 text-center rounded transition-all shrink-0 bg-black"
                  >
                    🔮 Profili Görüntüle
                  </Link>
                </div>
                ...
                <div className="md:col-span-2 flex flex-col gap-4">
                  <div><p className="text-white/30 text-xs tracking-widest uppercase">Tür</p><p className="text-white/70">{seciliKarakter.tur}</p></div>
                  <div><p className="text-white/30 text-xs tracking-widest uppercase">Köken</p><p className="text-white/70 capitalize">{seciliKarakter.koken}</p></div>
                </div>
              </div>
