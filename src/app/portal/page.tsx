...
export default function Portal() {
  ...
  return (
    <div className="h-screen bg-black overflow-hidden flex flex-col">
      ...
      {/* ÜST BAR */}
      <header className="relative z-10 shrink-0 flex items-center gap-6 px-6 py-3 border-b border-white/5">
        ...
        {ayFazi && (
          <div className={`flex items-center gap-2 px-3 py-1.5 border text-xs ${
            ...
            {ayFazi.dolunayMi && <span className="text-yellow-400/60 animate-pulse ml-1">◈ Geçitler Açık</span>}
          </div>
        )}
        ...
        <div className="ml-auto flex items-center gap-5">
          <Link href="/gorevler"
            className="flex items-center gap-2 text-cyan-400/40 text-xs tracking-widest uppercase hover:text-cyan-400 transition-all group">
          ...
          <Link href="/arsiv"
            className="flex items-center gap-2 text-fuchsia-400/50 text-xs tracking-widest uppercase hover:text-fuchsia-400 transition-all border border-fuchsia-400/20 hover:border-fuchsia-400/50 px-3 py-1.5 group">
            <span>✦</span>
            Kadim Arşiv
          </Link>
          <Link href="/arsiv/kisisel"
  className="flex items-center gap-2 text-violet-400/40 text-xs tracking-widest uppercase hover:text-violet-400 transition-all group">
  <span className="w-1.5 h-1.5 rounded-full bg-violet-400/30 group-hover:bg-violet-400 transition-all" />
  Hafıza
</Link>
          <Link href="/magaza"
  className="flex items-center gap-2 text-amber-400/40 text-xs tracking-widest uppercase hover:text-amber-400 transition-all border border-amber-400/15 hover:border-amber-400/40 px-3 py-1.5 group">
  <span>◈</span>
  Mağaza
</Link>
          <span className="text-white/10">·</span>
          <span className="text-fuchsia-400/25 text-xs tracking-widest uppercase">
            {profil?.rol === 'kabileli' ? '✦ Kabile' : 'Oyun Arkadaşı'}
          </span>
          {profil?.is_admin && (
      ...
      <div className="relative z-10 flex-1 flex overflow-hidden">
        ...
        {/* SAĞ — Tüm içerik, scroll yok */}
        <div className="flex-1 flex flex-col overflow-hidden pl-4 pr-8 py-6 gap-4">

          {/* Karakter başlık satırı */}
          {karakter && (
            <div className="shrink-0 flex items-baseline gap-6">
              <h1 className="text-4xl font-thin tracking-[0.25em] uppercase text-white"
              ...
              <span className={`ml-auto flex items-center gap-2 text-xs tracking-widest uppercase ${
          ...
          {karakter && (
            <div className="shrink-0 flex gap-0 border-b border-white/5">
              {([
                ...
                { key: 'gucler', label: 'Güçler & Zayıflıklar', renk: 'cyan' },
          ...
          {/* Alt bar — oyun anıları + dolunay */}
          <div className="shrink-0 flex items-center gap-8 pt-4 border-t border-white/5">
